...
> **This file is the source of truth.** `CLAUDE.md` beside it is a five-line
> generated stub that `@`-imports this one, because Claude Code reads
> `CLAUDE.md` and `agy` reads `AGENTS.md` natively. Edit here, then run
> `scripts/sync-agent-files.sh`. Never edit the generated file.
...
- `scripts/docker-gc.sh` — reclaims Docker disk no profile's lifecycle owns (stale VS Code devcontainers, one-off `docker run` shells, aged build cache). NEVER touches a `macolima-*` compose project, running or stopped: a stopped one means a profile is merely `down` and must stay resumable. Images and volumes are REPORTED, never pruned — an unfiltered `docker image prune` reaps the digest-pinned postgres/mongo/squid, because a `repo:tag@sha256:` pull stores them untagged and Docker calls that dangling. Prunes cache by AGE, not by a size cap — see the comment above its defaults.
- `scripts/workspace-scan.py` — host-side, read-only, stdlib-only scan of every git repo in every profile's workspace
  (plus host-only roots listed in the untracked `.workspace-scan.local`) against the work/0008 conventions: AGENTS.md as
  source with a `CLAUDE.md` stub, `.claude/settings*.json` hygiene, venv ownership and slots, `.local` ignores. Host-side
  because no container sees more than its own profile. **Its report names every repo in every profile and this repo is
  public**, so `--out` refuses any path git would track. Use a `*.local.*` name. The ask-fence check (`ASK-GAP`) is its
  own local model of Claude Code's documented Bash-rule matching (`rule_regex`): if the docs change, re-measure before
  trusting a clean result. `workspace-scan.test.sh` joins `test-offline`.
- `scripts/sync-agent-files.sh` — regenerates the thin `CLAUDE.md` beside every `AGENTS.md`. Two deliberate divergences from the sibling repo's copy, both in its header: it **refuses to overwrite a `CLAUDE.md` that is not already the generated stub** (theirs does an unconditional `cat >`, which is safe only because their migration already happened), and it **excludes vendored trees and test fixtures** — an `AGENTS.md` inside `sandbox_templates/skills/` is plugin payload, and one inside `scripts/depaudit-fixtures/` is the subject of a test. `--check` joins `test-offline`.
...
- **The environment names the venv** ([ADR-0013](docs/adr/0013-the-environment-names-the-venv.md)). `claude-agent` sets `UV_PROJECT_ENVIRONMENT=.venv-sandbox` in compose, so uv in the container builds and uses `<repo>/.venv-sandbox`; hosts leave it unset and keep `.venv`, which inside the container is the *host's* venv. Keep the value **relative** (it resolves per project root; absolute would put every repo in one venv), never add `UV_PYTHON` (it overrides each repo's `.python-version`), and don't re-add `.venv` to the deletion hook's disposable exception. `verify-sandbox.sh` asserts the exact value. `uv pip` ignores the variable, so it needs `--python .venv-sandbox`.
- **The sandbox notice is written into both agent homes, never into a repo** ([ADR-0015](docs/adr/0015-the-sandbox-briefs-agents-from-their-homes.md)). `profile.sh` syncs `sandbox_templates/common/agent-notice.md` into `claude-home/CLAUDE.md` and `gemini-home/config/rules/sandbox-notice.md` on every `up`/`recreate`/`rebuild`/`converge`; the marker is the neutral `managed by the sandbox — do not edit here`, shared with the sibling, and the sync script still recognises the two old sandbox-named markers so a migration replaces rather than stacks. Keep the text sandbox-neutral (`~`, not `/home/agent` or `/root`). `verify` hashes the template host-side and passes it in as `NOTICE_SHA`; `workspace-scan.py` flags any block found inside a repo (`NOTICE-IN-REPO`); `sync-agent-notice.sh --strip` retires one.
...
| `UV_PROJECT_ENVIRONMENT`, venv names (`.venv` vs `.venv-sandbox`), the hook's venv exception | `docs/adr/0013-the-environment-names-the-venv.md` |
| `sandbox_templates/bin/webfetch`, `[web-read]` hosts, adding a backend | `docs/web-read-broker.md` |
| `agent-notice.md`, `sync-agent-notice.sh`, where the notice is delivered | `docs/adr/0015-the-sandbox-briefs-agents-from-their-homes.md` + `sandbox_templates/antigravity/README.md` §"Briefed from" |
...
- [ ] Edited `AGENTS.md` (or added one in a new directory)? Run `just sync-agent-files` — `CLAUDE.md` is GENERATED and `test-offline` fails if it drifts. Never edit `CLAUDE.md` directly.
