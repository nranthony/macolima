...
Three paths — pick based on how you work.

**A. Attach to a running profile:** `scripts/profile.sh work up`, then `Cmd-Shift-P → Dev Containers: Attach to Running Container → claude-agent-work`. Simplest, and right when you don't care which folder opens.

**A2. Open a specific folder:** `just code <profile> <repo>` (`scripts/code-attach.sh`), e.g. `just code myproject engine`. Omit the repo to list what's under `/workspace`; trailing args go to `code` (`-r` reuses the current window). Use this when option A keeps reopening the wrong folder — it will, and not because you configured it wrong: VS Code records `(container + folder)` in its recent-window history, and **that history wins over the `workspaceFolder` key** in the attached-container configuration file described in B. Naming the folder in the URI bypasses history entirely. The script starts nothing and changes no container state; it only builds the `vscode-remote://attached-container+<hex>/<path>` URI and hands it to `code`. The container must already be up.

**B. Pin attach-time settings host-side.** *Attach to Running Container* does **not** read a repo's `.devcontainer/devcontainer.json` — that file is only consumed by *Reopen in Container*, which macolima doesn't use ([VS Code docs](https://code.visualstudio.com/docs/devcontainers/attach-container)). To customise the attached container — extensions, terminal, interpreter, `remoteUser` — use the host-side **attached-container configuration file**: `Cmd-Shift-P → Dev Containers: Open Attached Container Configuration File…`, keyed by image or container name. It supports a subset of devcontainer.json keys (`workspaceFolder`, `extensions`, `settings`, `forwardPorts`, `remoteUser`). **Python interpreter:** add `"settings": { "python.defaultInterpreterPath": "${workspaceFolder}/.venv-sandbox/bin/python" }`, so the Python extension picks the sandbox's venv rather than the repo's `.venv`, which is the host's ([ADR-0013](docs/adr/0013-the-environment-names-the-venv.md)). It applies only until an interpreter is picked, so a workspace that already remembers `.venv` needs re-picking once. For extensions that should land on *any* attached container, set `dev.containers.defaultExtensions` in host user `settings.json`. The container's security posture comes entirely from compose + seccomp + the image (agent runs as UID 1000, `cap_drop: ALL`, internal network) — never from a devcontainer.json.
...
The inside-container tripwire (`scripts/verify-sandbox.sh`) checks all three leakage paths plus the UID-0 orphan; if anything comes back from a reattach, it will FAIL loudly. The credential-helper check runs twice: once grepping the `GIT_CONFIG_GLOBAL` file directly, and once via `git config --show-origin --get-all credential.helper` so an injection at the *system* (`/etc/gitconfig`) or repo-*local* (`/workspace/.git/config`) layer is caught too — not just the global file. The SSH-socket check is gated on the *combination* of mitigations — an accumulating `/tmp/vscode-ssh-auth-*.sock` file is treated as cosmetic so long as `SSH_AUTH_SOCK` stays unset and `openssh-client` stays purged; FAIL only fires when one of those layers is also broken.

**Run it *after* a fresh reattach, not before.** Dev Containers applies its git/credential-sharing behaviour during the attach lifecycle, so a container that was already running when you changed the host settings can still reflect old state. The clean validation is: set the three host settings → fully reattach → `scripts/profile.sh <p> attach` → run `verify-sandbox.sh` from inside.
