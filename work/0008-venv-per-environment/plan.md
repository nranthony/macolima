...
| 5 | **Recreate every profile, one sitting;** `just verify <p>` each; then B6 (VS Code) | owner | **done 2026-09-11/13**: nranthony verified green in the sitting; the other three are proven by their `.venv-sandbox` builds under `/opt/uv` (2026-09-13 rebuilds ran with the variable set in jeremy_dahl and therapod too). B6 is in the image config ([notes](notes.md)) |
| 6 | **Repos, in final form, in one egress sitting:** the three therapod handoffs; depot T5; the other profiles' venvs build on first `uv sync` | repo agents; owner opens egress, then re-comments | **repos done 2026-09-14**: pipeline, misc_code, wearable_data_testing, ikigai ([notes](notes.md)). The done-check still exits 1 on one line: `.venv-linux` inside windows-ai-sandbox's managed notice block in ikigai's AGENTS.md, which no repo agent may edit — owner decision on which sandbox owns that block |
...
| `jeremy_dahl/jeremy_dahl_analytics` | after the conventions release: `docs/IN_TRANSIT.md` run lines → `uv run`; `.gitignore`. `.python-version` 3.12 landed 2026-09-13 | F: `.venv-linux/` (already gone, 2026-09-13) |
| `nranthony/ikigai` | **found by the 2026-09-13 scan, not the 2026-09-11 one.** Its `AGENTS.md` (inside the sibling's managed notice block) says `UV_PROJECT_ENVIRONMENT=.venv-linux uv sync`; six tracked `.venv/bin/python` allow rules; ~150 `.venv/bin` lines across skills, docstrings and run hints; an argparse-prefix hole in its deny fence. [handoff-ikigai.md](handoff-ikigai.md), delivered to `/Volumes/DataDrive/repo/nranthony/inbox/0008/` | its own `work/0023` proposal is superseded by the handoff |
