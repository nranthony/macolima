...
|---|---|---|
| `settings/template_diff` DRIFT | **RESOLVED 2026-09-01.** Read in full at last, and it was three things, none of them a real drift. `hooks` differed only by the template's `_comment` annotation — the `PreToolUse` wiring was byte-identical. `agentPushNotifEnabled` / `skipAutoPermissionPrompt` / `skipWorkflowUsageWarning` are written by Claude Code itself and were missing from the probe's strip list. `permissions.deny` had ONE surplus entry in the live profile, `Bash(glab:*)` — more restrictive than the template, for a binary this image does not install (W ships glab; M does not), so a stale seed. Probe now strips `_`-prefixed keys at every depth and knows the three agent-owned keys; the glab line was removed from the live profile. Verified `identical_after_strip: true`. | fixed — the probe was crying wolf, which is why it went four months unread |
