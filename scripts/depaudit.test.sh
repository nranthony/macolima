...
# macOS substrate: `mktemp -d` under $TMPDIR returns a /tmp/... path, but
# depaudit resolves every path with Path.resolve(), which follows macOS's
# /tmp -> /private/tmp symlink. A literal prefix comparison of the two then
# never matches and the roots assertions all fail for the wrong reason. Resolve
# at creation with `cd ... && pwd -P` (POSIX, bash-3.2 safe). Linux has no such
# symlink, which is why this is a macolima-only adaptation.
CLEANFIX="$(cd "$(mktemp -d "${TMPDIR:-/tmp}/n03clean.XXXXXX")" && pwd -P)"
...
NOISEFIX="$(cd "$(mktemp -d "${TMPDIR:-/tmp}/x04noise.XXXXXX")" && pwd -P)"
...
P01FIX="$(cd "$(mktemp -d "${TMPDIR:-/tmp}/p01.XXXXXX")" && pwd -P)"
...
WSFIX="$(cd "$(mktemp -d "${TMPDIR:-/tmp}/roots.XXXXXX")" && pwd -P)"
...
UPFIX="$(cd "$(mktemp -d "${TMPDIR:-/tmp}/unpinned.XXXXXX")" && pwd -P)"
printf 'torch==2.9.1\nnumpy>=1.26\npillow\nsafetensors\n# comment\n-r other.txt\n' \
