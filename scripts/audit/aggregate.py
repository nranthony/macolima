...
def _profile_from_hostname():
    """Container hostname is `claude-agent-<profile>` per docker-compose."""
    try:
        h = open("/etc/hostname").read().strip()
        if h.startswith("claude-agent-"):
            return h[len("claude-agent-"):]
    except OSError:
        pass
    return None
...
def _info():
    """§10/§11 informational fields the report needs verbatim."""
    info = {
        "stamp": datetime.datetime.now(datetime.timezone.utc)
                          .strftime("%Y-%m-%dT%H:%M:%SZ"),
        "profile": (os.environ.get("MACOLIMA_PROFILE")
