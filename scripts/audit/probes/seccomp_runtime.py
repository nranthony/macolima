"""§3b seccomp runtime probes via ctypes.
...
Note: clone3 has no glibc wrapper — glibc deliberately doesn't expose it
(it owns clone3 itself for fork()). Hence raw syscall() with arch-aware NR.
"""
import ctypes
import ctypes.util
import errno
...
#   aarch64: linux/include/uapi/asm-generic/unistd.h
# clone3, pidfd_open, close_range happen to share NRs across both arches.
_NR = {
...
def run():
    ...
    # unshare(CLONE_NEWUSER) — must EPERM. Use libc wrapper (arch-neutral).
    # On non-Linux libc (macOS — only when running this probe standalone for
    # debugging on the host), `unshare` isn't present; degrade to UNKNOWN.
    try:
        ...
        out.append({
            ...
            "details": {
                ...
                "note": "unshare not available in this libc (probably non-Linux)",
            },
        })

    # clone3(NULL, 0) — MUST be ENOSYS=38, NOT EPERM. Raw syscall.
    ...
    # getpgid(0) — bash job control. Use os wrapper (arch-neutral).
    try:
        pgid = os.getpgid(0)
    ...
    # Linux-only symbol; degrade to UNKNOWN on non-Linux libc.
    try:
        ...
        out.append({
            ...
            "details": {
                "error": f"{type(ex).__name__}: {ex}",
                "note": "getxattr not available (probably non-Linux libc)",
            },
        })
