"""§9 network egress, DNS, DB sibling reachability.
...
def run():
    ...
    if proxy_up:
        ...
        # This is an intentional control test of our own infrastructure.
        try:
        ...
        # M1: GET on a non-Safe_port (allowed host:8080) — must be 403.
        # Tests `http_access deny !Safe_ports` (audit M1).
        try:
            line = _http_via_proxy("GET", "api.anthropic.com", 8080, "/")
            out.append(_check(
                "M1_non_safe_port_blocked",
                _status_code(line) in ("403", "400"),
                response=line,
                rationale="audit M1: http_access deny !Safe_ports",
            ))
        except Exception as e:
            out.append({
                "section": "network",
                "name": "M1_non_safe_port_blocked",
                "verdict": "UNKNOWN",
                "details": {"error": f"{type(e).__name__}: {e}"},
            })

        # H1: CONNECT on port 80 — must be 4xx, MUST NOT tunnel.
        # Tests `http_access deny CONNECT !SSL_ports` (audit H1). A real
        # regression here would return "HTTP/1.1 200 Connection established".
        try:
            line = _connect_via_proxy("api.anthropic.com", 80)
            code = _status_code(line)
            out.append(_check(
                "H1_connect_80_blocked",
                code in ("403", "400"),
                response=line,
                rationale="audit H1: http_access deny CONNECT !SSL_ports",
            ))
        except Exception as e:
            out.append({
                "section": "network",
                "name": "H1_connect_80_blocked",
                "verdict": "UNKNOWN",
                "details": {"error": f"{type(e).__name__}: {e}"},
            })
    ...
    # External DNS MUST be sinkholed (audit H2). Docker's embedded resolver
    # at 127.0.0.11 forwards to host DNS regardless of `internal: true`,
    # which would be a DNS exfil channel. Fix: dns: [127.0.0.1] sinkhole +
    # extra_hosts. Test: any external name should NXDOMAIN.
    try:
        socket.gethostbyname("example.com")
        out.append(_check(
            "external_dns_blocked", False,
            observed="example.com resolved (DNS exfil channel open)",
            rationale="audit H2: must NOT resolve external names",
        ))
    except (socket.gaierror, OSError):
        out.append(_check("external_dns_blocked", True))

    # Internal DNS MUST resolve via /etc/hosts (extra_hosts entries).
    for name in ["egress-proxy", "postgres", "mongo"]:
        try:
