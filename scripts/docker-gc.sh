...
# =============================================================================
# Usage:
#   scripts/docker-gc.sh [--dry-run] [--yes] [--days N] [--cache-age DURATION]
#
# Examples:
...
#   - BuildKit cache older than --cache-age (default 168h / 7 days)
#
...
#   Sandbox-managed containers (compose project `macolima-*`), running or
...
# CACHE PRUNE IS BY AGE HERE, NOT BY SIZE, and that is a measured divergence
# from windows-ai-sandbox rather than a preference. Their `--keep-storage=4g`
# sets what may REMAIN, and this image's own build cache is ~8.4 GB — so a 4 GB
# cap evicts the base layers on every run. Measured in this repo (work/0001 §4):
# with the size cap a `build --refresh-ai` went 22s -> 97s -> 97s with 0 cached
# layers; under `--filter until=168h` it is 21s/21s/21s with 6 cached. A GC that
# silently destroys the cache the build depends on is worse than no GC.
# `profile.sh build` prunes the same way for the same reason.
dry=0; assume_yes=0; days=30; cache_age=168h
while [ $# -gt 0 ]; do
  case "$1" in
    --dry-run|-n)  dry=1 ;;
    --yes|-y)      assume_yes=1 ;;
    --days)        days="${2:?--days needs a value}"; shift ;;
    --cache-age)   cache_age="${2:?--cache-age needs a value}"; shift ;;
    -h|--help)     sed -n '2,/^set -euo/p' "${BASH_SOURCE[0]}" \
                     | grep '^#' | sed 's/^# \{0,1\}//'; exit 0 ;;
    *) fail "unknown flag '$1' (valid: --dry-run --yes --days N --cache-age DURATION)" ;;
  esac
  shift
done
...
info "Docker GC — removing stopped containers older than ${days}d, build cache older than ${cache_age}"
echo
...
while IFS=$'\t' read -r name project; do
  [ -n "$name" ] || continue
  case "$project" in
    macolima-*) skipped_sandbox=$((skipped_sandbox + 1)); continue ;;
  esac
...
ok "build cache older than ${cache_age} dropped"
