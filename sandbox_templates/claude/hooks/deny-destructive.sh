...
#     /home/agent/.cache are disposable by design, .venv-sandbox and node_modules
...
RM_SEGMENTS
  if [ "$_rm_ask" = 1 ]; then
    emit_ask "rm-file" "deleting a file is a human step here, single files included and even when an approved plan names them. List the exact paths and wait for confirmation. Disposable targets (under /tmp, /home/agent/.cache, .venv-sandbox, node_modules, __pycache__, the *_cache dirs, build/dist, *.pyc) do not reach this prompt — if you are cleaning those up, say so and use those paths."
  fi
fi
