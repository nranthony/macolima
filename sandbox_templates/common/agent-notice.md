## ⚠️ This repo may be edited by an agent inside `macolima`
...
- **What persists vs. what vanishes.** `/workspace` and your git commits persist
  across container recreates. `/tmp`, `/home/agent/.local`, `/home/agent/.npm-global` are
...
- **Persists**: `/workspace` (the repo tree) and the agent home
  (`/home/agent/.claude` — skills at `~/.claude/skills/<name>/SKILL.md`, plugins and
  marketplaces at `~/.claude/plugins/`, standing instructions at
  `~/.claude/CLAUDE.md`), plus `/home/agent/.config` and `/home/agent/.cache`. These are
...
- **Dies and cannot execute**: `/tmp`, `/home/agent/.local`, `/home/agent/.npm-global` are
...
`just`); `rg`, `find`, `jq`; `webfetch` for web reads; `myclickup` for ClickUp
reads and writes. Plan with installs, network widening, and remote git as human
steps.

There is no GPU on this host and that is not a fault to investigate: it is a
Colima VM on macOS with no device passthrough. `nvidia-smi` is absent, CUDA is
absent, and "no GPU" is the CORRECT answer, not a misconfiguration to work
around.
